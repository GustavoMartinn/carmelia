...
export function UrlBar() {
  ...
  const handleSend = async () => {
    ...
    setError(null)
    try {
      const result = await ExecuteRequest(rawContent, activeProject.activeEnv, activeProject.path, {})
      setResponse(result)
      if (result.error) {
  ...
  const handleSave = async () => {
    if (!activeFile || !isDirty || !activeProject) return
    try {
      await SaveRequestToProject(activeProject.path, activeFile, rawContent)
      useAppStore.getState().setOriginalContent(rawContent)
    } catch (err: any) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-700 bg-gray-800">
      ...
      <button
        ...
        className="px-4 py-1.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed rounded text-sm font-medium transition-colors flex items-center gap-1.5"
      >
        {loading ? (
